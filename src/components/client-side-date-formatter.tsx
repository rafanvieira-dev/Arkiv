...
export function ClientSideDateFormatter({ isoDateString, placeholderText = "...", formatString = 'dd/MM/yyyy HH:mm' }: ClientSideDateFormatterProps) {
  ...
  useEffect(() => {
    if (isoDateString && isValid(parseISO(isoDateString))) {
      try {
        ...
        setFormattedDate(format(parseISO(isoDateString), formatString, { locale: ptBR }));
      } catch (e) {
  ...
  }, [isoDateString, placeholderText, formatString]);
